...
//   - the "Other Projects" footer with link cards
//
  ...
  return (
    <section className="relative h-[344px] w-full overflow-hidden">
      ...
      <div className="absolute top-[24px] left-[24px] flex items-center gap-[8px]">
        <Link href="/" className="font-display text-cream text-[16px] leading-[24px] hover:underline">
          c stavridis
        </Link>
...
// Pigment selector pill on the right side of the hero. Visual only — clicks
// do nothing because the active pigment is fixed per project route.
...
// ---------------------------------------------------------------------------
// "Other Projects" footer — low-opacity Washes wash with link cards.
// ---------------------------------------------------------------------------
        ...
        <div className="flex flex-wrap gap-[24px]">
          {otherProjects.map((p) => (
            <Link
              key={p.href + p.label}
              href={p.href}
              className="group relative block h-[224px] w-[368px] overflow-hidden rounded-[12px] bg-white transition-transform hover:scale-[1.02]"
            >
              <div
                aria-hidden="true"

...
// supplies:
//   - the full-bleed hero strip (1280×344) with a low-opacity Washes wash
//     tinted by the project's pigment, the c-stavridis brand mark, a
//     Close-Project link back to `/`, a (decorative) pigment-selector pill,
//     and a centered title + tagline
//   - the "Other Projects" footer with link cards to siblings
//
// The shell renders at 1280px wide centered on a cream canvas and breaks
// out of the parent `<main>`'s `max-w-3xl` constraint with the same
// `relative left-1/2 w-screen -translate-x-1/2` pattern the landing page
// uses.
// ---------------------------------------------------------------------------
...
export function ProjectPageShell({
  ...
  return (
    <div className="bg-cream text-coffee-deep relative left-1/2 w-screen -translate-x-1/2 overflow-hidden">
...
function ProjectHero({
  ...
  useEffect(() => {
    ...
    const splashTimer = window.setTimeout(() => {
      ...
      wash.splash([{ x: rect.width / 2, y: rect.height * 0.6, velocity: 35 }], 'spray', {
  ...
  return (
    <section className="relative h-[344px] w-full overflow-hidden">
      {/* Low-opacity wash background — cream paper showing through. */}
      ...
      <div className="absolute top-[24px] left-[24px] flex items-center gap-[8px]">
        <span className="font-display text-[16px] leading-[24px] text-black">c stavridis</span>
        ...
        <span
          className="font-body text-[16px] leading-[24px] text-[#7d7d7d]"
      ...
      <div className="absolute top-[24px] right-[24px] flex items-center gap-[16px]">
        <Link
          href="/"
          className="text-coffee-deep font-mono text-[12px] leading-[24px] hover:underline"
        >
          Close Project
        </Link>
      ...
      <div className="absolute top-[254px] left-1/2 flex w-[560px] -translate-x-1/2 flex-col items-center gap-[12px] text-center">
        <h1 className="font-display text-[32px] leading-[36px] whitespace-pre-line text-black">
          {title}
        </h1>
        <p className="text-coffee-deep font-mono text-[12px] leading-[24px]">{tagline}</p>
      </div>
    </section>
  );
}
...
function OtherProjectsFooter({
  ...
  return (
    <section className="relative w-full overflow-hidden px-6 pt-[72px] pb-[96px]">
      <div ref={hostRef} className="absolute inset-0 opacity-25" aria-hidden="true" />
      <div className="relative flex flex-col gap-[24px]">
        <h2 className="font-body text-coffee-deep text-[24px] leading-[32px] font-bold">
          Other Projects
        </h2>
        <div className="flex flex-wrap gap-[24px]">
          {otherProjects.map((p) => (
            <Link
              key={p.href}
              href={p.href}
              className="group relative block h-[228px] w-[368px] overflow-hidden rounded-[12px] bg-white shadow-[0_2px_12px_rgba(33,30,31,0.06)] transition-transform hover:scale-[1.02]"
            >
              <div
                aria-hidden="true"
